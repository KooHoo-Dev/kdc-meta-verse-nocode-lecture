...
| **MouseShooter** | `Aim At Crosshair` · `Aim Distance` · `Fire Button` · `Hold To Fire` | — | — |
| **Projectile** | `Life Time` · `Target Tag` | — | `On Hit` · `On Miss` |
| **MuzzleFlashPlayer** | `Particle` · `Flash Light` · `Duration` | `Play` | — |

> 📝 **총과 입력이 나뉘어 있습니다** (2026-08-23 결정 10)
>
> `SimpleGun` 은 **총알을 총구 방향으로 내보내는 일만** 합니다.
> **겨누고 쏘라고 알려주는 일**은 `MouseShooter` 가 합니다. 같은 그릇에 붙습니다.
>
> **32차시에서 `MouseShooter` 를 떼어냅니다.** 그 자리에 손이 들어옵니다.
> VR 쪽은 새 부품 없이 `XR Grab Interactable` 의 **`Activated`** 칸에 `SimpleGun.Fire` 를 겁니다.

> ✅ **MuzzleFlashPlayer 는 존치로 결정됐습니다** (2026-08-23 결정 11 · 함정 ③ 종결)
>
> `ParticleSystem.Play()` 는 **드롭다운에 나옵니다.** 그런데 이 부품에는
> **`Flash Light` 를 `Duration` 동안 켰다 끄는** 기능이 있어서 내장 기능으로 대체되지 않습니다.
...
- 총알은 **언제나 `MuzzlePoint` 가 향한 쪽**으로 나갑니다. 예외 없습니다.
  총이 화면 구석에 있어 십자선과 어긋나는 문제는 **`MouseShooter` 가 총을 돌려서** 해결합니다.
...
## 6. ⚠️ 마우스 조작 검증 체크리스트 (U-7) — 항목 원본

> **이 문서에서 가장 중요한 절입니다.** 아래가 **46개 항목의 원본**입니다.
> ❌ 가 나오는 항목이 **곧 컴포넌트가 흡수해야 할 명세**입니다.

> ✏️ **판정을 채우는 곳은 여기가 아닙니다**
>
> **`plans/05_U6_U7_에디터_작업지시.md` §3** 을 채워주세요.
> 같은 46항목에 **판정 기준("이렇게 되면 ✅")과 조율값 기록란**이 더해져 있습니다.
>
> **항목을 추가·삭제할 때는 두 문서를 같이 고쳐야 합니다.**
...
| 4 | **U-7 검증 체크리스트 채우기** | `plans/05` §3 판정 표 | **가장 중요** |
...
**순서 1~2 는 끝났습니다.** 함정 ①②③ 결론이 확정됐고, **컴포넌트 14개도 제작 완료**입니다.

순서 3~6 은 에디터 작업이라 사용자가 하셔야 합니다.
→ 단계별 순서와 채우는 체크리스트는 **`plans/05_U6_U7_에디터_작업지시.md`** 에 있습니다.

**체크리스트를 채워서 주시면** 순서 5·8 을 이어서 진행하겠습니다.
