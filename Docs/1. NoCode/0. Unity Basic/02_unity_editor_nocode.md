...
!!! info "이 자료는 이렇게 보세요"
    ...
    - 이번 차시에도 **코드를 한 줄도 쓰지 않습니다.** 1차시와 똑같아요.
    - 오늘 하실 일은 **창 열고 닫기, 마우스로 화면 돌려보기, 색 고르기** 정도가 전부입니다.
    - 1차시에서 "지금은 몰라도 됩니다"라고 넘어갔던 **그 복잡한 화면**을, 오늘 하나씩 뜯어봅니다.
...
1차시에 Unity에는 얼굴이 두 개라고 했었죠. **Editor**와 **Runtime**이요.
...
- 여기서 **우클릭 → 새 물건을 만듭니다**
- 물건을 **다른 물건 위로 끌어다 놓으면 한 덩어리처럼 묶입니다** — 이건 **3차시**에서 제대로 다룹니다
...
!!! tip "1차시에서 해보셨던 그 방식 그대로입니다"
    붙이는 것도 **끌어다 놓기**, 버튼에 거는 것도 **`On Click ()` 에 연결**입니다.
    새로 배우실 게 없어요. **연습 한 번 더 한다고 생각하시면 됩니다.**
...
!!! abstract "목표"
    **버튼을 눌러 Console에 메시지를 찍어보고, 빨간 줄도 일부러 내봅니다.**

1. 강의자료의 **`ConsoleTester`** 를 **Hierarchy의 큐브 위로 끌어다 놓습니다.**
2. Inspector에서 **`Message`** 칸에 아무 말이나 입력합니다. 예: `안녕하세요`
3. **Hierarchy** 우클릭 → **`UI`** → **`Button - TextMeshPro`**
   (처음이라면 설치 창이 뜹니다 → **`Import TMP Essentials`** 를 눌러주세요. 1차시에 하셨던 그것입니다.)
4. 버튼을 선택하고 Inspector를 아래로 내려 **`On Click ()`** 을 찾습니다.
5. **`+`** 클릭 → **큐브를 왼쪽 칸에 끌어다 놓기** → 오른쪽 목록에서 **`ConsoleTester` → `SendLog`**
6. **`Console` 탭을 클릭해서 열어둡니다.**
7. **▶** 를 누르고 버튼을 클릭합니다.
   → **Console에 하얀 줄로 메시지가 뜹니다.**
8. ▶ 를 끄고, Inspector의 **`Log Type`** 을 **`Error`** 로 바꾼 뒤 다시 ▶ → 버튼 클릭
   → **이번엔 빨간 줄로 뜹니다.**
9. Console 왼쪽 위의 **`Clear`** 를 눌러 싹 지웁니다.
...
!!! warning "잘 안 되신다면"
    - 목록에 `SendLog`가 안 보인다 → 5번에서 **큐브를 왼쪽 칸에 끌어다 놓지 않으신 경우**가 대부분입니다. 왼쪽이 `None`이면 목록이 비어 있습니다.
    - Console에 아무것도 안 뜬다 → **▶ 를 누른 상태에서** 버튼을 클릭하셔야 합니다.
...
이제 어디에 뭐가 있는지 아시니까, 3차시부터는 **훨씬 편해지실 거예요.** 충분히 잘하셨습니다.

---

## **13. 다음 차시 준비**

!!! example "3차시 — 세 창을 오가며 만들기"
...
!!! note "미리 해두시면 좋은 것"
    ...
    - [ ] 강의자료의 **`CubeSpinner`** 파일이 프로젝트에 들어 있는지 확인 (1차시에 쓰던 그 부품을 다시 씁니다)
