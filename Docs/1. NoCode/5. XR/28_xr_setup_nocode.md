...
**고글 흉내를 내주는 부품**이 따로 있거든요. 이름은 **XR Device Simulator**입니다.

| 진짜 고글이라면 | 흉내 내는 부품이라면 |
| :--- | :--- |
| 고개를 돌린다 | **마우스를 움직인다** |
| 걸어간다 | **키보드를 누른다** |
| 손을 뻗는다 | **키를 눌러 손을 고르고 마우스로 움직인다** |

!!! success "그러니까 안심하셔도 됩니다"
...
!!! success "11차시에 했던 말이 여기서 돌아옵니다"
    **"한 번 만들면 어디든 간다."**

    VR도 마찬가지입니다. 공통 약속에 맞춰 만들어두면 **기기가 바뀌어도 다시 안 만들어도 됩니다.**

!!! note "OpenXR이라는 이름은 안 외우셔도 됩니다"
...
| **XR Device Simulator** | **고글 흉내 내는 부품** — 키보드·마우스로 조작 |
...
1. `File` → `New Scene` → 기본 틀로 새 씬을 만들고, **`28_VR연습`** 으로 저장합니다.
   → 오늘은 여기서만 작업합니다.
2. 위쪽 **▶** 를 눌러봅니다.
...
2. **`Starter Assets`** 의 **`Import`** 를 누릅니다.
3. **`XR Device Simulator`** 의 **`Import`** 를 누릅니다. ← **고글 흉내 내는 부품이 여기 있습니다**
...
1. `Edit` → `Project Settings` → 왼쪽 목록에 **`XR Plug-in Management`** 가 **새로 생겼습니다.**
   → 실습 ①에서 없던 그 항목입니다.
2. **`Install XR Plug-in Management`** 버튼이 보이면 누릅니다.
3. 위쪽에 **작은 탭 두 개**가 있습니다. **컴퓨터 모양 탭**(PC용)이 골라져 있는지 확인합니다.
4. **`OpenXR`** 에 **체크**합니다.
   → 잠시 뒤 아래에 항목이 몇 개 더 생깁니다.
...
2. 목록에 **`XR Origin`** 이 생깁니다. **왼쪽 화살표를 눌러 펼쳐보세요.**
   → 안에 **Camera** 와 **손 두 개**가 들어 있습니다. **5절에서 본 그 구조**입니다.
3. 같이 생긴 다른 것이 있어도 **지우지 마세요.** VR에 필요한 것들입니다.

#### **3단계 — 고글 흉내 부품 놓기**

1. **재료함(`Project`)** 에서 **`XR Device Simulator`** 를 찾습니다.
2. 그것을 **Hierarchy 로 끌어다 놓습니다.**
...
flowchart TB
    ...
    K -->|예제를 꺼낸다| S2["XR Device Simulator<br/>고글 흉내"]
...
??? success "정답 보기"
    ...
    **고글 흉내 내는 부품(XR Device Simulator)** 이 있어서, **키보드와 마우스로** 대신할 수 있습니다.
...
| **XR Device Simulator** | 고글 흉내 내는 부품 |
| **XR Origin** | VR 속의 나 (머리와 두 손) |
| **Controller** | VR 조종기 = 손 |
