...
4. **`Interaction Layer Mask`** 를 클릭합니다.

    > ⚠️ **목록에 `Teleport` 가 없다면 — 먼저 만들어야 합니다**
    >
    > 목록에 **`Default`** 하나만 보이시나요? **정상입니다.** 아직 안 만들었거든요.
    >
    > 목록 맨 아래 **`Edit Layers...`** 를 누르면 빈 칸이 쭉 나옵니다.
    > **`Default` 아래 첫 번째 빈 칸**에 **`Teleport`** 라고 적으세요. 그게 전부입니다.
    >
    > **21차시의 이름표(Tag)와 완전히 같습니다.** 거기서도 `Add Tag...` 로 먼저 만들고 골랐죠.
    > **만들어야 고를 수 있습니다.**

5. 이제 목록에서 **`Default` 를 끄고 `Teleport` 를 켭니다.** ← **이걸 빼먹으면 안 됩니다**
6. **▶** 를 누르고, **`I`** 를 **누른 채로** 바닥을 겨눕니다.
   → **선이 휘어지면서 도착 지점이 표시됩니다.**
7. **`I` 를 떼면** 그 자리로 **툭 이동합니다.**
...
> ⚠️ **순간이동 선이 빨갛다면**
>
> **`Interaction Layer Mask` 를 `Teleport` 로 안 바꾸신 겁니다.** 3단계 5번이요.
> 목록에 `Teleport` 가 아예 없으면 **4번의 `Edit Layers...` 부터** 하셔야 합니다.
>
