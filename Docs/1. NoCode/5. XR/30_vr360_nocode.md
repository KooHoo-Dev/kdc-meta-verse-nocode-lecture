...
2. 조종석에서 **`Texture Shape`** 가 **`2D`** 인지만 확인합니다. → **바꿀 것 없습니다.**

> ✅ **손댈 게 없습니다**
>
> 360 사진은 넣기만 하면 **그대로 쓸 수 있습니다.** 설정을 만질 필요가 없어요.

> 💡 **혹시 하늘이 흐릿하게 보인다면**
>
> 조종석 아래쪽 **`Max Size`** 를 보세요. **`2048`** 로 되어 있을 겁니다.
>
> 사진이 그보다 크면 **여기서 줄여버립니다.** 크기를 올리고 맨 아래 **`Apply`** 를 누르면 선명해져요.
> **강의자료의 사진은 이 값에 맞춰뒀으니 그냥 두셔도 됩니다.**
...
3. 만든 재료를 고르고, 조종석 **맨 위 목록**에서 **`Skybox`** → **`Panoramic`** 을 고릅니다.
   → **재료의 종류를 고르는 목록**입니다. 뜻은 몰라도 됩니다.
    ...
    > ⚠️ **`Skybox` 안에 네 가지가 있습니다**
    >
    > **`6 Sided` · `Cubemap` · `Panoramic` · `Procedural`** 이 나옵니다.
    > **반드시 `Panoramic`** 을 고르세요. 다른 걸 고르면 사진이 제대로 안 감깁니다.

4. **`Spherical (HDR)`** 칸에 1단계의 사진을 **끌어다 놓습니다.**
   → 재료 종류를 `Panoramic` 으로 바꾸면 생기는 **큰 사진 칸**입니다.

> 📝 **밑에 칸이 몇 개 더 있는데**
>
> **`Mapping`** 과 **`Image Type`** 이 보일 거예요.
