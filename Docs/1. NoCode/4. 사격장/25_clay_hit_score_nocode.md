...
> 🟨 **이 부품은 실습 ② 에서만 씁니다**
>
> 효과 파일이 준비되면 다시 정리할 예정이라, **지금은 안 붙이셔도 됩니다.**
...
> 🟨 **실습 ② 만 아직 확정 전입니다**
>
> 실습 ① · ③ · ④ 는 **에디터에서 확인을 마쳤습니다.**
> **실습 ②(터지는 효과)** 는 기본 도형으로 만들면 보기에 좋지 않아,
...
> 🟨 **이 실습은 다시 쓸 예정입니다**
>
> 아래 순서대로 하면 효과가 만들어지긴 합니다. 다만 기본 도형으로 만든 거라
> 보기에 좋지 않습니다. **제대로 만든 효과 파일**이 준비되면 그걸 쓰는 쪽으로 바꿉니다.
>
> **건너뛰셔도 됩니다.** 맞으면 사라지는 것도, 점수가 오르는 것도 그대로 됩니다.
...
1. `Canvas` 위에서 우클릭 → `UI` → `Text - TextMeshPro` → 이름을 **`ScoreText`** 로
   → 앵커를 **`top-right`** 로 (`Alt` 누르고), 한글 글꼴을 지정합니다.
2. **`ScoreText`** 에 강의자료의 **`ValueDisplay`** 를 끌어다 놓습니다.
...
3. `Prefabs` 의 **`Target` 을 더블클릭**해 들어갑니다.
4. `Hittable` 의 `On Hit` 에 **`+`** 를 하나 더 만듭니다.
5. **방금 만든 `ScoreText` 를 왼쪽 칸에 끌어다 놓으려고 해보세요.**
   → **안 들어갑니다.** 칸이 아예 받지 않아요.

<details>
<summary>✅ 왜 안 될까요</summary>
...
6. **`<`** 로 나옵니다.
7. **`GameManager`** 를 선택합니다. **23차시에 만들어두셨어요.**
   → 없으면 **Hierarchy** 우클릭 → `Create Empty` → 이름을 **`GameManager`** 로
8. 강의자료의 **`HitScorer`** 를 끌어다 놓고, **`Points Per Hit`** 을 **`10`** 으로

#### **4단계 — 화면에 이어줍니다**

9. `HitScorer` 의 **`On Score Changed`** 에서 **`+`** 클릭
10. **`ScoreText` 를 왼쪽 칸에 끌어다 놓기**
11. 오른쪽 목록 → **위쪽 `Dynamic float`** 의 **`ValueDisplay`** → **`SetValue`**

    > ⚠️ **아래쪽 `Static Parameters` 를 고르면 안 됩니다**
    >
    > 아래쪽을 고르면 **오른 점수가 아니라 미리 적어둔 고정 숫자**가 들어갑니다.
    > 18차시에 슬라이더로 겪었던 그 함정이에요.
...
> 🎯 **목표**
>
> **못 맞히고 사라진 표적**도 세어보려고 합니다. 23차시의 `On Expired` 를 씁니다.
>
> **끝까지는 안 됩니다.** 막히는 게 오늘 마지막으로 볼 것이에요.

1. `Canvas` 에 글자를 하나 더 만듭니다. 이름은 **`MissText`**
2. **`ValueDisplay`** 를 붙이고 `Prefix` 를 **`놓침: `** 으로
3. `Prefabs` 의 **`Target` 을 더블클릭**해 틀 고치는 방으로 들어갑니다.
4. **`TargetLife` 의 `On Expired`** 에 **`+`** 를 하나 만듭니다.
5. **씬의 `MissText` 를 왼쪽 칸에 끌어다 놓으려고 해보세요.**
   → **또 안 들어갑니다.**

<details>
<summary>✅ 여기서 막히는 게 정상입니다</summary>

**틀에서 씬의 글자를 부를 방법이 없습니다.**

실습 ③에서 배운 그 제약이 **여기서 또 나옵니다.**

> 💡 **"점수 담당을 하나 더 두면 되지 않나요?"**
>
> `HitScorer` 는 **한 그릇에 하나만** 붙게 만들어져 있습니다. 붙이려 하면 Unity 가 거절해요.
>
> 표적이 알아서 찾아가는 담당이 **둘이면 어느 쪽으로 갈지 정해지지 않기** 때문입니다.
> 아까 말씀드린 **"심판은 하나"** 가 부품 안에도 들어 있는 거예요.
...
> - [ ] 씬을 **`25_완성` 로 복사**해두기 (**게임이 되는 첫 버전이라 특히 중요합니다**)
