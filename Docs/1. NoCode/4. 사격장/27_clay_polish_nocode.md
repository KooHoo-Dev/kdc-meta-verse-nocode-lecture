...
> 💡 **기본은 `Audio Source` 만으로 됩니다**
>
> Unity 내장 부품인 `Audio Source` 의 **`Play`** 가 목록에 그대로 나옵니다.
> 오늘 실습도 그걸로 합니다.
>
> 강의자료의 **`AudioPlayer`** 는 **소리를 더 자연스럽게 만들고 싶을 때** 씁니다.

### **AudioPlayer (더 하고 싶다면)**
...
> 🟨 **소리 파일이 아직 없습니다**
>
> 순서는 **에디터에서 확인을 마쳤습니다.** 연결 방식도 그대로 쓰시면 됩니다.
> 다만 **강의자료의 소리 파일**이 아직 준비 중이라, 지금은 소리가 안 납니다.
> 파일이 들어오면 `Audio Clip` 칸에 끼우기만 하면 됩니다.
...
5. 목록 → **`AudioSource`** → **`Play`**
    ...
    > 💡 **총소리가 기계처럼 들린다면**
    >
    > 같은 소리가 똑같이 반복되면 금방 질립니다.
    > 강의자료의 **`AudioPlayer`** 를 대신 붙이고 **`Random Pitch`** 를 `0.1` 쯤 주면
    > 쏠 때마다 **높낮이가 조금씩 달라져** 훨씬 자연스러워집니다.
