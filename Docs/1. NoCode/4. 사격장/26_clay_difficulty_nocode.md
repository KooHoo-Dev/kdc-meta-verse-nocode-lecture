...
!!! warning "🟨 이 절은 아직 확정 전입니다"
    아래 순서는 **잠정 초안**입니다.
    특히 **실습 ④의 커서 흐름**은 에디터 검증(U-7 `5-5` ~ `5-10`) 결과로 확정됩니다.
...
10. **`DifficultyText`** 를 넣고, 목록 → **위쪽 `Dynamic string`** 의 **`TextMeshProUGUI`** → **`text`**
    ...
    > 🟨 **U-7 `5-9` 결과로 확정** — 목록에 `Dynamic string` 이 나오는지
    > 안 나오면 이 단계를 빼고 실습을 마칩니다.
...
> 🟨 **U-7 `5-7` · `5-10` 결과로 확정** — 커서 잠금과 패널 숨기기가 되는지

8. **▶** 를 누르고 **난이도 고르기 → 시작 버튼 → 플레이** 순서로 해봅니다.
