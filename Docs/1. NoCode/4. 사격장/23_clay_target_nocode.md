...
2. `Scale` 을 **`1, 0.05, 1`** 로 → **납작한 접시**가 됩니다.
3. **`Add Component`** → **`Rigidbody`** 추가
   → **`Collision Detection`** 을 **`Continuous`** 로 바꿔둡니다.
   → 24차시에 총알이 뚫고 지나가지 않게 하는 설정입니다. 그때 다시 설명합니다.
...
> - 너무 안 맞는다 → `Scale` 의 **첫 칸과 끝 칸(X · Z)** 을 키워보세요. 접시가 커져서 맞히기 쉬워집니다.
