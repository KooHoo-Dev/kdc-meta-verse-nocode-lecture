...
??? success "무슨 일이 일어났나요"
    **버튼으로 물건을 움직이고, 돌리고, 키우고, 되돌리고 있습니다.**
    ...
    - **↑ ↓** 를 누르면 → **두 번째(Y)** 가 변하고요
    - **⟳** 를 누르면 → `Rotation` 의 **두 번째(Y)** 가 변합니다
...
7. **▶** 를 누르고 이동 버튼을 눌러봅니다. → **누를 때마다 글자가 갱신됩니다.**

??? success "확인해보세요"
    - 버튼을 누를 때마다 글자가 **새로 그려지나요?**
      → `On Changed` 가 **바뀔 때마다 실행**되고 있다는 뜻입니다.
